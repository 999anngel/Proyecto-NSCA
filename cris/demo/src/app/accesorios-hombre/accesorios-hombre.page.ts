...
@Component({
  selector: 'app-Accesorios-hombre',
  templateUrl: './Accesorios-hombre.page.html',
  styleUrls: ['./Accesorios-hombre.page.scss'],
})
export class AccesoriosHombrePage implements OnInit {
  ...
  async getProducts() {
    ...
    await this.restApi.getProducts().subscribe({
      next: (res) => {
        // Filtrar productos por el género 'Hombre'
        this.productosFiltrados = res.filter((producto: ClProducto) => 
          producto.genero === 'Hombre' && producto.categoria === 'Accesorios'
        );
        loading.dismiss();
      },
      error: (err) => {
