...
@Component({
  selector: 'app-zapatillas-hombre',
  templateUrl: './zapatillas-hombre.page.html',
  styleUrls: ['./zapatillas-hombre.page.scss'],
})
export class ZapatillasHombrePage implements OnInit {
    ...
    await this.restApi.getProducts().subscribe({
      next: (res) => {
        ...
        this.productosFiltrados = res.filter((producto: ClProducto) => 
          producto.genero === 'Hombre' && producto.categoria === 'Zapatillas'
        );
        loading.dismiss();
      },
      error: (err) => {
