...
@Component({
  selector: 'app-Ropa-unisex',
  templateUrl: './Ropa-unisex.page.html',
  styleUrls: ['./Ropa-unisex.page.scss'],
})
export class RopaUnisexPage implements OnInit {
  ...
  async getProducts() {
    ...
    await this.restApi.getProducts().subscribe({
      next: (res) => {
        // Filtrar productos por el género 'Ropa'
        this.productosFiltrados = res.filter((producto: ClProducto) => producto.categoria === 'Ropa');
        loading.dismiss();
      },
      error: (err) => {
