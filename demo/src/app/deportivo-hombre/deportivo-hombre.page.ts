...
@Component({
  selector: 'app-Deportivo-hombre',
  templateUrl: './Deportivo-hombre.page.html',
  styleUrls: ['./Deportivo-hombre.page.scss'],
})
export class DeportivoHombrePage implements OnInit {
    ...
    await this.restApi.getProducts().subscribe({
      next: (res) => {
        ...
        this.productosFiltrados = res.filter((producto: ClProducto) => 
          producto.genero === 'Hombre' && producto.categoria === 'Deportivo'
        );
        loading.dismiss();
      },
      error: (err) => {
