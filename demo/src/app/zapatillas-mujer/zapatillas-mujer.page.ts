...
@Component({
  selector: 'app-zapatillas-mujer',
  templateUrl: './zapatillas-mujer.page.html',
  styleUrls: ['./zapatillas-mujer.page.scss'],
})
export class ZapatillasMujerPage implements OnInit {
    ...
    await this.restApi.getProducts().subscribe({
      next: (res) => {
        ...
        this.productosFiltrados = res.filter((producto: ClProducto) => 
          producto.genero === 'Mujer' && producto.categoria === 'Zapatillas'
        );
        loading.dismiss();
      },
      error: (err) => {
