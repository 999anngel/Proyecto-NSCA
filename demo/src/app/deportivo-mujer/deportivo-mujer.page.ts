...
@Component({
  selector: 'app-Deportivo-Mujer',
  templateUrl: './Deportivo-Mujer.page.html',
  styleUrls: ['./Deportivo-Mujer.page.scss'],
})
export class DeportivoMujerPage implements OnInit {
    ...
    await this.restApi.getProducts().subscribe({
      next: (res) => {
        ...
        this.productosFiltrados = res.filter((producto: ClProducto) => 
          producto.genero === 'Mujer' && producto.categoria === 'Deportivo'
        );
        loading.dismiss();
      },
      error: (err) => {
