...
@Component({
  selector: 'app-zapatillas-unisex',
  templateUrl: './zapatillas-unisex.page.html',
  styleUrls: ['./zapatillas-unisex.page.scss'],
})
export class ZapatillasUnisexPage implements OnInit {
  ...
  async getProducts() {
    ...
    await this.restApi.getProducts().subscribe({
      next: (res) => {
        ...
        this.productosFiltrados = res.filter((producto: ClProducto) => producto.categoria === 'Zapatillas');
        loading.dismiss();
      },
      error: (err) => {
