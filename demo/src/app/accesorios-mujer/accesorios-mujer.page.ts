...
@Component({
  selector: 'app-Accesorios-mujer',
  templateUrl: './Accesorios-mujer.page.html',
  styleUrls: ['./Accesorios-mujer.page.scss'],
})
export class AccesoriosMujerPage implements OnInit {
  ...
  async getProducts() {
    ...
    await this.restApi.getProducts().subscribe({
      next: (res) => {
        // Filtrar productos por el género 'Mujer'
        this.productosFiltrados = res.filter((producto: ClProducto) => 
          producto.genero === 'Mujer' && producto.categoria === 'Accesorios'
        );
        loading.dismiss();
      },
      error: (err) => {
